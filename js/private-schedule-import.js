(function () {
  ...
  const minutes = (entry) => {
    if (!entry.start || !entry.end || /^frei$/i.test(entry.title)) return 0;
    const [startHour, startMinute] = entry.start.split(":").map(Number); const [endHour, endMinute] = entry.end.split(":").map(Number);
    return Math.max(0, endHour * 60 + endMinute - startHour * 60 - startMinute - Number(entry.break || 0));
  };
  ...
  function install() {
    ...
    const renderScheduleViews = (shifts) => {
      ...
      dayPanel.innerHTML = `<div class="date-switch"><span>${selectedDate.toLocaleDateString("de-DE", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}</span></div>${selected ? `<article class="my-shift"><p><i class="fa-solid fa-briefcase"></i> Eigener Dienstplan</p><div><i class="fa-regular fa-clock"></i><span><strong>${/^frei$/i.test(selected.title) ? "Frei" : `${escapeHtml(selected.start)} – ${escapeHtml(selected.end)}`}</strong><small>${escapeHtml(selected.title || "Arbeit")}</small></span><em>Importiert</em></div><footer>${/^frei$/i.test(selected.title) ? "Kein Einsatz geplant" : `${duration(minutes(selected))} Arbeitszeit · ${Number(selected.break || 0)} Min. Pause`}</footer></article>` : '<p class="private-import-empty">Noch kein eigener Dienstplan übernommen.</p>'}`;
      const monday = new Date(selectedDate); const weekday = (monday.getDay() + 6) % 7; monday.setDate(monday.getDate() - weekday); const weekDates = Array.from({ length: 7 }, (_, index) => { const value = new Date(monday); value.setDate(monday.getDate() + index); return value; });
      ...
      weekPanel.innerHTML = `<div class="date-switch"><span>${weekDates[0].toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit" })} – ${weekDates[6].toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit", year: "numeric" })}</span></div><div class="week-summary"><span>Arbeitszeit<strong>${duration(total)}</strong></span><span>Einsätze<strong>${weekEntries.filter((entry) => minutes(entry) > 0).length}</strong></span><span>Pausen<strong>${weekEntries.reduce((sum, entry) => sum + Number(entry.break || 0), 0)} Min.</strong></span><span>Freie Tage<strong>${weekEntries.filter((entry) => !minutes(entry)).length}</strong></span></div><div class="week-list">${weekEntries.map((entry) => { const date = new Date(`${entry.date}T12:00:00`); const free = !minutes(entry); return `<div class="week-row${free ? " free" : ""}"><b>${date.toLocaleDateString("de-DE", { weekday: "short" })}<small>${date.toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit" })}</small></b><i></i><span>${escapeHtml(entry.title || "Arbeit")}</span><em>${free ? "" : `${Number(entry.break || 0)} Min. Pause`}</em><strong>${free ? "Frei" : `${escapeHtml(entry.start)} – ${escapeHtml(entry.end)}`}</strong></div>`; }).join("")}</div>`;
