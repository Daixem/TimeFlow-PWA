(function () {
  ...
  const dailyTarget = (config = settings()) => {
    const weeklyHours = Number(config.weeklyTargetHours);
    const workdays = Math.max(1, Math.min(7, Number(config.regularWorkDays || 5)));
    return weeklyHours > 0 ? Math.round(weeklyHours * 60 / workdays) : Number(config.dailyTargetMinutes || 480);
  };
  const weeklyTarget = (config = settings()) => {
    const weeklyHours = Number(config.weeklyTargetHours);
    return weeklyHours > 0 ? weeklyHours * 60 : dailyTarget(config) * Math.max(1, Math.min(7, Number(config.regularWorkDays || 5)));
  };
  ...
  function scheduleDueForMonth(month, today = currentDate()) {
    ...
    entriesByDate.forEach((entry) => {
      ...
      // Frei, Urlaub und Krankheit sind ausdrücklich sollzeitfreie Tage.
      if (isCreditedAbsence(entry)) return;
      contractualDays += 1;
    });
    const normalFreeDays = 7 - Math.max(1, Math.min(7, Number(settings().regularWorkDays || 5)));
    const extraFreeDays = [...freeByWeek.values()].reduce((sum, freeDays) => sum + Math.max(0, freeDays - normalFreeDays), 0);
    // The configured number of free days per Monday–Sunday week is normal. Every other entered day
    ...
    const contractualTarget = contractualDays * dailyTarget();
    return { hasSchedule: true, targetDue: contractualTarget, absenceCredit, extraFreeDays, contractualDays, normalFreeDays };
  }
  ...
  function monthlyValues(month = currentMonth()) {
    ...
    const configuredMonthlyTarget = Number(targets[month] ?? settings().monthlyTargetHours ?? 0);
    const target = Math.round(configuredMonthlyTarget > 0 ? configuredMonthlyTarget * 60 : weeklyTarget() * 52 / 12);
  ...
  function install() {
    ...
    form.addEventListener("submit", async (event) => { event.preventDefault(); const hours = Number(form.elements.hours.value || 0); const minuteValue = Number(form.elements.minutes.value || 0); const value = hours * 60 + minuteValue; if (!value) return; const type = form.elements.entryType.value; const work = type === "manual_work"; const adjustment = work ? 0 : Number(form.elements.direction.value) * value; const entry = { id: `manual-${Date.now()}`, date: form.elements.date.value, minutes: work ? value : 0, target: 0, adjustment, note: form.elements.note.value.trim() || (work ? "Manuell erfasste Arbeitszeit" : type === "opening_balance" ? "Anfangssaldo" : "Manuelle Korrektur"), source: "manual", entryType: type };
      if (window.TimeFlowWorkTimeServerEnabled?.() && !work) {
        try {
          const client = window.TimeFlowWorkTimeApi.create({ storage: platform().storage }); const current = await client.getCurrent(); const currentState = current.state && typeof current.state === "object" ? current.state : {}; const corrections = Array.isArray(currentState.manualCorrections) ? currentState.manualCorrections : []; const result = await client.writeChange({ eventType: "TIME_CORRECTION", state: { ...currentState, manualCorrections: [...corrections, entry] } });
