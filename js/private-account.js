(function () {
  ...
  function captureCompletedWorkday() {
    ...
    entries.push({ id, date: new Date(workday.workStart).toLocaleDateString("sv-SE"), minutes: net, target, adjustment: 0, note: "Stempelung", source: "stamp", entryType: "stamped_work" }); write(entries); document.dispatchEvent(new CustomEvent("timeflow:private-account-updated"));
  }
  function monthlyValues(month = currentMonth()) {
    ...
    const targetDue = targetDueForMonth(month, target);
    const calculated = { stamped, manual, target, targetDue, balance: stamped + manual - targetDue };
  ...
  function rememberCurrentMonth(values) { platform().storage.setItem(STATE_KEY, JSON.stringify({ month: currentMonth(), values: { stamped: values.stamped, manual: values.manual, target: values.target, targetDue: values.targetDue, balance: values.balance } })); }
  ...
  function install() {
    ...
    function render() { const entries = read().sort((a, b) => `${b.date}${b.id}`.localeCompare(`${a.date}${a.id}`)); const month = monthPicker.value || currentMonth(); const { stamped, manual, target, targetDue, balance, entries: monthEntries } = monthlyValues(month); renderHomeMonth();
  ...
  window.TimeFlowPrivateAccount = { refreshHome: renderHomeMonth, captureCompletedWorkday, targetDueForMonth };
