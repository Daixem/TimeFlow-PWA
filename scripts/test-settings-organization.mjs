...
for (const marker of ["Support & Feedback", "Letzter Einsatz", "Nächster Einsatz", "Arbeits- & Jugendschutz", "MutationObserver", 'lang.startsWith("ar")', "aria-description", "input[type=submit]", "timeflow:language-changed", "isKnownRendering", "function locale()", "toLocaleDateString", "toLocaleTimeString", '"Guten Abend":"Good evening"', '"Mein eigener Dienstplan":"My personal schedule"', '"Was möchtest du öffnen?":"What would you like to open?"', '"Mittwoch":"Wednesday"']) if (!localization.includes(marker)) throw new Error(`Vollständige UI-Lokalisierung fehlt: ${marker}`);
...
for (const marker of ["css/settings-organization.css?v=__TIMEFLOW_BUILD__", "js/settings-organization.js?v=__TIMEFLOW_BUILD__", "css/adaptive-ui-fixes.css?v=__TIMEFLOW_BUILD__", "js/ui-localization.js?v=__TIMEFLOW_BUILD__", "register(\"sw.js\""]) if (!index.includes(marker)) throw new Error(`Asset fehlt: ${marker}`);
