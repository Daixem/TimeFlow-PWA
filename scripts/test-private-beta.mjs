...
for (const marker of ["timeflow-personalization-v1", "data-custom-background-input", "prepareBackground", "indexedDB", "--tf-custom-background-image", "BACKGROUND_MAX_BYTES", "image/webp", "image/png", "image/jpeg"]) if (!personalization.includes(marker) && !theme.includes(marker)) throw new Error(`Eigenes Hintergrundbild fehlt: ${marker}`);
for (const marker of ["background-size:cover", "rgb(var(--tf-custom-background-rgb)/.32)"]) if (!theme.includes(marker)) throw new Error(`Bildmodus-Darstellung fehlt: ${marker}`);
